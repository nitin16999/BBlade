...
const AdminService = () => {
    ...
    function goBack() {
        setaddServiceScreen(true)
        setallServiceScreen(false)
        setstatScreen(false)
    }

    function goToStat() {
        setallServiceScreen(false)
        setaddServiceScreen(false)
        setstatScreen(true)
    }

    function goToAddService() {
        setallServiceScreen(true)
        setaddServiceScreen(false)
        setstatScreen(false)
        clearAddService()
        //getData()
    ...
    async function getData() {
        await firestore().collection("Services").onSnapshot(snapshot => {
                ...
                if (v2 != '') {
                    if (regex2.test(v2) == true) {
                        if (selectedHour != '0' || selectedMin != '0') {
                            const userRole = "Admin"//await AsyncStorage.getItem('@user_role')
                            if (userRole == "Admin") {



                                firestore().collection("Services").doc().set({
                                    name: serviceName,
                                    price: servicePrice,
                                    ...
                                    data.map((value, index) => {
                                        return (
                                            <CardView
                                                ...
                                                <View>
                                                    <TouchableOpacity style={{ width: wp('24%'), backgroundColor: '#000', borderRadius: 25, paddingVertical: 8, alignSelf: 'center', marginTop: 20, marginHorizontal: 15 }} onPress={() => deleteData(toString(value.name))}>
